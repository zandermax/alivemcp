...
This index is intended to be generated by `scripts/generate_wiki_index.py`.

Run the generator locally (dry-run):

```bash
python scripts/generate_wiki_index.py
```

To actually write files under `docs/wiki/`:

```bash
python scripts/generate_wiki_index.py --write
```

You can add per-tool pages under `docs/wiki/tools/<domain>/<tool>.md`.
...
- [get_clip_notes](tools/midi/get_clip_notes.md) _(see also: Clips)_
...
- [remove_notes](tools/midi/remove_notes.md) _(see also: Clips)_
...
- [set_device_param](tools/devices/set_device_param.md) _(possible alias — see note above)_
...
- [flatten_track](tools/tracks/flatten_track.md) — renders a track's device chain to audio in place _(also listed under ⚠️ Destructive)_
- [quantize_clip](tools/clips/quantize_clip.md) — reads note data and writes it back time-corrected _(also listed under ✏️ Write › Clips)_
- [quantize_clip_pitch](tools/clips/quantize_clip_pitch.md) — reads pitch data and writes it back corrected _(also listed under ✏️ Write › Clips)_
