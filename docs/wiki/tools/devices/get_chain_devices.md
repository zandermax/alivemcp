...
summary: ""
Live mapping: "- Reads `device.chains[chain_index].devices` and returns `name`, `class_name`, and `is_active` for each device."
---
...
```json
{
  "ok": true,
  "chain_index": 0,
  "devices": [{ "name": "Synth", "class_name": "Operator", "is_active": true }],
  "count": 1
}
```
